...
async function initCardButton() {
  ...
  if (!window.Stripe) {
    ...
    s.onload = () => initCardButton();
    document.head.appendChild(s);
    return;
  }

  const stripe = window.Stripe(PAY_CONFIG.stripePublishableKey);
...
window.addEventListener('DOMContentLoaded', () => {
  // Show default section (from hash or 'about')
  const hash = (location.hash || '').replace('#','');
  const defaultSection = hash || 'about';

  document.querySelectorAll('.main-section').forEach(section => {
    section.style.display = section.id === defaultSection ? 'block' : 'none';
  });

  // Mark active tab (if matches), otherwise set first tab
  const tabs = Array.from(document.querySelectorAll('.tab-nav a'));
  if (tabs.length) {
    let activeSet = false;
    tabs.forEach(t => {
      const sec = t.getAttribute('data-section');
      if (sec === defaultSection) { t.classList.add('active'); activeSet = true; }
    });
    if (!activeSet) tabs[0].classList.add('active');
  }

  // Tabs click handler (SPA: show one section, update hash)
  document.querySelectorAll('.tab-nav a').forEach(tab => {
    tab.addEventListener('click', (e) => {
      ...
      whenPaymentsShown(sectionId);
    });
  });

  // Initial loads (so data is ready when user switches to tab)
  // Load teams initially (so list is ready even if user switches later)
  loadTeamsAndRender();
  initPhotoUpload();
  listPhotos();

  // Start Swish / Stripe only if payments tab is visible now
  whenPaymentsShown(defaultSection);

  // Optional auto-refresh for teams
